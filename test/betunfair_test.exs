defmodule BetunfairTest do
  ...
  test "user_create_deposit_get" do
    ...
    user = new_user("u1")
    assert {:ok,u1} = Betunfair.user_create(user,"Francisco Gonzalez")
    assert {:error,_} = (Betunfair.user_create(user,"Francisco Gonzalez"))
    assert {:ok,_} = (Betunfair.user_deposit(u1,2000))
    assert {:error,_} = (Betunfair.user_deposit(u1,-1))
    assert {:error,_} = (Betunfair.user_deposit(u1,0))
    assert {:error,_} = (Betunfair.user_deposit("u11",0))
