          ...
          if stake > user.balance do
            ...
            case Betunfair.Repo.get(Betunfair.Market, market_id) do
              nil ->
                {:error, "user with id #{user_id} doesn't exist"}
              _market ->
                #TODO: remaining_stake??? -> matchmaking algo?
                changeset = Betunfair.Bet.changeset(%Betunfair.Bet{}, %{user_id: user_id, market_id: market_id, original_stake: stake, remaining_stake: 999, odds: odds, type: type})
    ...
    def handle_call({:bet_cancel, bet_id}, _from, state) do
      case Betunfair.Repo.get(Betunfair.Bet, bet_id) do
        nil ->
          {:error, "Could not find the bet with id #{bet_id}", state}
        bet ->
          changeset = Betunfair.Bet.changeset(bet, %{remaining_stake: 0}) #TODO: is this how i cancel a bet?
